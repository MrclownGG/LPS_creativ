...
## 10. 多语言文案替换（2025-12-02 新增）

- 生成 / 预览落地页时，前端会在生成页面中选择页面语言（当前支持：中文 / 英文 / 葡萄牙语），并通过 `language` 字段传给后端；  
- 后端在渲染模板 HTML 时会：  
  - 根据 `language` 调整 `<html lang="...">` 属性；  
  - 按固定映射表将模板中的中文文案（如“免费网站”“下载”“更多精彩影片进APP搜索”）替换为对应语言版本；  
  - 写入静态文件后，将选用的语言记录到 `landing_page.language` 字段；  
- 工作流详情页通过 `landing_pages[].language` 展示每个落地页的语言；  
- 工作流列表页通过 `languages` 字段汇总展示该批次是「中文 / 英文 / 葡萄牙语 / 多语言」。  
