...
- `poster_url TEXT`：海报图 URL，必填。可以是外部图片链接，也可以是本地上传后生成的路径（例如 `/generated/video_posters/{id}/poster.jpg`）
...
- `status VARCHAR(30)`：状态
  - 流程：`draft` → `generating` → `pending_ad` → `ready` → `in_use` → `archived`
    - `draft`：草稿，尚未生成落地页；  
    - `pending_ad`：已生成落地页，等待上传广告图；  
    - `ready`：落地页与广告图都准备完成，可以被投放计划引用；  
    - `in_use`：至少被一个投放计划引用。当前通过业务接口 `POST /api/campaigns/{id}/workflows` 维护：一旦有任意 campaign 关联该 workflow，即自动从 `ready` 进入 `in_use`；当不再被任何 campaign 关联时，则自动从 `in_use` 恢复为 `ready`；  
    - `archived`：已归档，仅保留历史记录，不再参与生成或投放。  
...
- `language VARCHAR(10)`：页面语言代码，例如 `zh` / `en` / `pt`，默认 `zh`  
- `created_at TIMESTAMP`：创建时间，默认 `NOW()` 
...
- `language VARCHAR(10)`：页面语言代码，例如 `zh` / `en` / `pt`，默认 `zh`。用于区分同一批次下不同语言版本的落地页，并在前端列表/详情中展示语言信息。
...
## 13. 补充说明：`landing_page.language` 字段

- 字段位置：`landing_page.language`  
- 类型：`VARCHAR(10)`  
- 作用：记录该落地页渲染时使用的页面语言代码，用于前端在列表 / 详情中展示语言信息，并区分同一批次下不同语言版本的落地页。  
- 当前实现（自 2025-12-02 起）：  
  - 生成接口 `POST /api/workflows/{id}/generate` 接收 `language` 字段（`zh` / `en` / `pt`），并在创建 `landing_page` 记录时写入此值；  
  - 预览接口 `POST /api/workflows/preview` 仅使用 `language` 控制 HTML 文案替换，不写入数据库；  
  - 未传或传入非法值时，统一回退为 `zh`。  
- 注意：  
  - 前端展示时会将 `zh` / `en` / `pt` 分别映射为“中文 / 英文 / 葡萄牙语”；  
  - 若后续增加语言种类，只需在后端支持新的语言代码并同步更新前端映射与相关文档。

### 附：广告图文件落盘与下载

- 广告图均保存于 `backend/generated/workflow_ad_images/{workflow_id}`，文件名为上传时生成的 UUID。
- `workflow_ad_map` 记录 `workflow_id` 与 `ad_image_id` 映射，新接口会根据该映射收集文件后打包下载。
- 若目录缺失或文件被运维清理，`GET /workflows/{workflow_id}/ad-images/download` 会返回 404 提示重新上传。
