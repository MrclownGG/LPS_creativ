...
## 2025-11-27：工作流广告图状态联动 + 投放计划模块落地（workflow 复用）

### 1）后端改动：广告图与状态联动

- `POST /api/workflows/{id}/ad-images/upload` 现在复用依赖注入的数据库会话，上传成功后：  
  - 将广告素材写入 `ad_image_library`，并与对应 workflow 建立 `workflow_ad_map` 关联；  
  - 若 workflow 状态仍为 `pending_ad`，立即自动切换为 `ready`，不再需要人工“标记完成”。  
- `GET /api/workflows` 与 `GET /api/workflows/{id}` 的响应新增 `ad_image_count` 字段，用于返回当前已经绑定的广告图数量。

### 2）后端改动：投放计划与工作流映射 + `in_use` 状态

- 新增投放计划相关 API（文件：`backend/app/api/campaigns.py`）：  
  - `GET /api/campaigns`：按状态和分页查询投放计划列表，返回每个计划已关联的工作流数量 `workflow_count`；  
  - `POST /api/campaigns`：创建投放计划，包含名称、渠道数组 `channels`、地区数组 `regions` 等；  
  - `GET /api/campaigns/{id}`：获取单个投放计划详情，附带 `workflows: [{id, name, status}]` 列表；  
  - `POST /api/campaigns/{id}/workflows`：为指定投放计划批量设置关联的 workflow（整包覆盖）。  
- `POST /api/campaigns/{id}/workflows` 的业务规则：  
  - 仅允许状态为 `ready` 或 `in_use` 的 workflow 被传入；  
  - 会先清空该计划原有的 `campaign_workflow_map` 记录，再写入新的关联；  
  - 完成写入后，会对所有“旧关联 ∪ 新关联”的 workflow 做一次引用计数，并根据是否被至少一个计划引用自动维护：  
    - 若某个 workflow 当前引用计数 > 0，且状态为 `ready`，则自动更新为 `in_use`；  
    - 若某个 workflow 当前引用计数 = 0，且状态为 `in_use`，则自动恢复为 `ready`。  
- `GET /api/workflows` 与 `GET /api/workflows/{id}` 的响应中新增：  
  - `campaign_count`：当前被多少个投放计划引用；  
  - `campaign_names: string[]`：所有引用该 workflow 的投放计划名称列表。

### 3）前端改动：工作流列表 / 详情 + 投放计划列表

- 工作流列表页（`WorkflowListPage`）：  
  - 状态列第二行现在展示：  
    - `广告图：X 张 ｜ 已关联投流计划：计划A / 计划B`；  
    - 若未关联任何计划，则显示“尚未关联投流计划”。  
  - 状态筛选中保留 `in_use` 选项，对应“已投放”状态。  
- 工作流详情页（`WorkflowDetailPage`）：  
  - 在当前状态一行下方增加一行说明：“已关联投流计划：计划A / 计划B”，或“尚未关联投流计划”；  
  - 上传广告图成功后，仍然会刷新详情与广告图列表，联动状态从 `pending_ad` → `ready`。  
- 投放计划列表页（`CampaignListPage`）：  
  - 选择关联批次时，弹窗中展示所有状态为 `ready` 或 `in_use` 的工作流，支持复用同一个 workflow 到多个投放计划；  
  - 新增“查看详情”按钮，点击后会调用 `GET /api/campaigns/{id}`，在 Modal 中展示：  
    - 计划基本信息（渠道、地区、创建人、创建时间等）；  
    - 已关联工作流列表（ID、名称、当前状态 Tag）。

### 4）文档同步

- `docs/接口约定.md`：  
  - 在“工作流广告图管理”小节中补充 `ad_image_count`、`campaign_count`、`campaign_names` 的说明；  
  - 新增 “2.19 投放计划与工作流关联（POST /api/campaigns/{id}/workflows）” 小节，记录请求体结构、整包覆盖规则以及 `ready` / `in_use` 状态联动策略。  
- `docs/数据库设计说明.md`：  
  - 在 `workflow.status` 的说明中补充 `in_use` 状态的语义：  
    - 代表“已被一个或多个投放计划引用”；  
    - 通过业务层自动在 `ready` 与 `in_use` 之间转换，不直接通过手工更新。  

## 2025-12-02：落地页多语言支持 + 语言展示

1）后端：

- `landing_page` 表新增字段：`language VARCHAR(10) NOT NULL DEFAULT 'zh'`，通过 Alembic 迁移 `8c3f9a1b2d34_add_language_to_landing_page` 落地；  
- `POST /api/workflows/{id}/generate`：请求体新增 `language` 字段（`zh` / `en` / `pt`），生成落地页时写入 `landing_page.language`；  
- `POST /api/workflows/preview`：同样接收 `language`，仅用于控制 HTML 文案替换，不落库；  
- `GET /api/workflows/{id}`：`landing_pages[]` 元素新增 `language` 字段，用于详情页显示每个落地页语言；  
- `GET /api/workflows`：每条 workflow 新增 `languages: string[]`，为该批次所有落地页的语言集合（去重），用于列表页语言汇总展示。

2）前端：

- `WorkflowGeneratePage`：在 Step 2 下方增加“页面语言”下拉框（中文 / 英文 / 葡萄牙语），调用 `generateWorkflow` 与 `previewWorkflow` 时传入 `language`；  
- `WorkflowDetailPage`：在“生成页面”列中为每个落地页展示语言 Tag（中文 / 英文 / 葡萄牙语），预览 iframe 保持不变；  
- `WorkflowListPage`：新增“语言”一列，基于 `languages` 字段显示「中文 / 英文 / 葡萄牙语 / 多语言 / 暂无落地页」。

3）注意：

- 多语言仅针对模板中的固定文案（例如“免费网站”“下载”“更多精彩影片进APP搜索”），使用后端维护的映射表，不依赖外部翻译服务；  
- 后续如需增加语言，只需扩展后端支持的语言代码、更新映射表，并同步调整前端下拉选项与文档说明。

## 2025-12-04 渠道 token 扩展 + 落地页 UI 调整

### 1. 渠道 token 扩展
- `/channels/{channel_id}/token` 新增 `external_channel_id`（外部 c_id）字段，供前端展示和落地页注入。
- WorkflowGeneratePage 的 Step3 重新启用，支持播放量排序、多模板校验，并把渠道 c_id/token 结果同步展示。

### 2. 落地页预览与下载
- WorkflowDetailPage 视频素材卡片可换行显示，右侧 iframe 预览区域放大。
- “下载落地页包”切换为按钮样式，同时新增“下载广告图包”，调用后端打包接口。
- 后端新增 `GET /workflows/{workflow_id}/ad-images/download`，把 `backend/generated/workflow_ad_images/{workflow_id}` 内文件打成 zip 返回。
