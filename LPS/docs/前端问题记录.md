...
## 2025-12-04 渠道选择与预览体验优化

### WorkflowGeneratePage
- 恢复 Step3 渠道板块，支持按播放量排序视频、多选模板前置校验以及强制选定渠道/查询 token。
- “查询 token” 成功后会展示渠道 ID、渠道 c_id 以及 token，便于核对落地页注入信息。

### WorkflowDetailPage
- 视频素材列改为宽度 320px 的流式卡片布局，勾选顺序会清晰展示。
- 右侧 iframe 预览区域放大，手机框可铺满整列，文字说明统一移动到卡片底部。
- 新增“下载落地页包”“下载广告图包”按钮，其中广告图按钮会触发后端打包下载接口。
